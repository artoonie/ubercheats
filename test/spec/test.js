...
describe('Test background', () => {
  it('check stored data', () => {
    ...
    expect(data.actualFloatMi).toEqual(5.5);
  ...
  it('check mi to km conversions', () => {
    ...
    expect(floats.actualFloatMi).toEqual(5.5);
    expect(Math.abs(floats.uberPaidForFloatMi-1.367017)).toBeLessThan(.001);

    floats = bg.distanceStringsToFloatsInMi("1.609334 km", "1.0 mi", 0);
    expect(Math.abs(floats.actualFloatMi-1.0)).toBeLessThan(.001);
    expect(Math.abs(floats.uberPaidForFloatMi-1.0)).toBeLessThan(.001);
  });
})
